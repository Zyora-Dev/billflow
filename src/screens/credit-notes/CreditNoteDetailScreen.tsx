...
export default function CreditNoteDetailScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  const fetchCN = async () => {
    try {
      const r = await api.get(`/api/credit-notes/${id}`);
      setCn(r.data);
    } catch {}
  };

  useEffect(() => { fetchCN(); }, [id]);
  useEffect(() => { navigation.addListener('focus', fetchCN); }, [navigation]);

  const changeStatus = async (status: string) => {
    try {
      await api.patch(`/api/credit-notes/${id}/status?status=${status}`);
  ...
  const handleDelete = () => {
    Alert.alert('Delete Credit Note', 'Are you sure? This will reverse any invoice adjustments.', [
      { text: 'Cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await api.delete(`/api/credit-notes/${id}`); navigation.goBack(); } catch (e: any) {
          Alert.alert('Error', e.response?.data?.detail || 'Failed');
        }
      }},
    ]);
  ...
  if (!cn) return <View style={s.center}><Text style={{ color: colors.gray500 }}>Loading...</Text></View>;
  ...
  const accent = statusAccent[cn.status] || colors.primary;

  const discAmt = cn.discount_type === 'percentage'
    ? (cn.subtotal || 0) * (cn.discount_value || 0) / 100
    : (cn.discount_value || 0);
    ...
    <ScrollView style={s.container}>
      ...
      <View style={s.header}>
        <View style={s.headerTop}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.headerLabel}>CREDIT NOTE</Text>
            <Text style={s.headerNumber}>{cn.cn_number}</Text>
          </View>
          <View style={[s.statusPill, { backgroundColor: accent + '15', borderColor: accent + '40' }]}>
            <View style={[s.statusDot, { backgroundColor: accent }]} />
            <Text style={[s.statusText, { color: accent }]}>{cn.status}</Text>
          </View>
        </View>

        <View style={s.amountBlock}>
          <Text style={s.amountLabel}>Total Amount</Text>
          <Text style={s.amountValue}>₹{Number(cn.total || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text>
        </View>

        <View style={s.dateRow}>
          <View style={s.dateCol}>
            <Text style={s.dateLabel}>Date</Text>
            <Text style={s.dateValue}>{dateFmt(cn.cn_date)}</Text>
          </View>
          <View style={s.dateDiv} />
          <View style={s.dateCol}>
            <Text style={s.dateLabel}>Items</Text>
            <Text style={s.dateValue}>{(cn.items || []).length}</Text>
          </View>
          <View style={s.dateDiv} />
          <View style={s.dateCol}>
            <Text style={s.dateLabel}>Status</Text>
            <Text style={[s.dateValue, { color: accent }]}>{cn.status}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={s.quickActions}>
        ...
        {cn.status === 'Draft' && (
          <TouchableOpacity style={s.qAction} onPress={() => navigation.navigate('CreditNoteForm', { id })}>
      ...
      {cn.reason ? (
        <View style={s.section}>
          ...
          <Text style={s.reasonText}>{cn.reason}</Text>
        </View>
      ) : null}
      ...
      <View style={s.section}>
        ...
        <View style={s.custCard}>
          <View style={s.custAvatar}>
            <Text style={s.custAvatarText}>{(cn.customer_name || '?').charAt(0).toUpperCase()}</Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={s.custName}>{cn.customer_name || 'N/A'}</Text>
          </View>
        </View>
      </View>
        ...
        <View style={s.section}>
          <View style={s.sectionHead}>
            <View style={s.sectionTitleRow}>
              ...
              <Text style={s.sectionTitle}>Linked Invoice</Text>
            </View>
          </View>
          <View style={s.linkedCard}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.linkedNumber}>{cn.invoice_number}</Text>
              <Text style={s.linkedSub}>Invoice linked to this credit note</Text>
            </View>
      ...
      {cn.place_of_supply ? (
        <View style={s.section}>
          ...
          <Text style={s.posText}>{cn.place_of_supply}</Text>
        </View>
      ) : null}
      ...
      <View style={s.section}>
        <View style={s.sectionHead}>
          ...
          <View style={s.itemCountChip}>
            <Text style={s.itemCountText}>{(cn.items || []).length}</Text>
          </View>
        </View>
        {(cn.items || []).map((item: any, i: number) => (
          <View key={i} style={[s.lineItem, i === (cn.items || []).length - 1 && { borderBottomWidth: 0 }]}>
      ...
      <View style={s.section}>
        ...
        <View style={s.sumRow}><Text style={s.sumLabel}>Subtotal</Text><CurrencyText amount={cn.subtotal} style={s.sumValue} /></View>
        {cn.discount_value > 0 && (
          <View style={s.sumRow}>
            <Text style={s.sumLabel}>Discount {cn.discount_type === 'percentage' ? `(${cn.discount_value}%)` : ''}</Text>
            <Text style={[s.sumValue, { color: colors.danger }]}>−₹{discAmt.toFixed(2)}</Text>
          </View>
        )}
        {cn.tax_amount > 0 && <View style={s.sumRow}><Text style={s.sumLabel}>Tax (CGST + SGST)</Text><CurrencyText amount={cn.tax_amount} style={s.sumValue} /></View>}
        <View style={[s.sumRow, s.totalRow]}><Text style={s.totalLabel}>Total</Text><CurrencyText amount={cn.total} style={s.totalValue} /></View>
      </View>
      ...
      {cn.notes ? (
        <View style={s.section}>
          ...
          <Text style={s.notes}>{cn.notes}</Text>
        </View>
      ) : null}
      ...
      <View style={s.moreActions}>
        {cn.status === 'Draft' && (
          <TouchableOpacity style={s.moreBtn} onPress={() => navigation.navigate('CreditNoteForm', { id })}>
            <Ionicons name="create-outline" size={18} color={colors.gray600} />
            <Text style={s.moreBtnText}>Edit Credit Note</Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity style={[s.moreBtn, { borderColor: colors.danger + '40' }]} onPress={handleDelete}>
          <Ionicons name="trash-outline" size={18} color={colors.danger} />
          <Text style={[s.moreBtnText, { color: colors.danger }]}>Delete Credit Note</Text>
        </TouchableOpacity>
      </View>
        ...
        <TouchableOpacity style={s.modalOverlay} activeOpacity={1} onPress={() => setShowStatusPicker(false)}>
          <View style={s.statusModal}>
            ...
            {STATUS_OPTIONS.map(st => (
              <TouchableOpacity key={st} style={[s.statusOption, cn.status === st && { backgroundColor: colors.primary + '10' }]} onPress={() => changeStatus(st)}>
                <StatusBadge status={st} />
                {cn.status === st && <Ionicons name="checkmark-circle" size={20} color={colors.primary} />}
              </TouchableOpacity>
            ))}
          </View>
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  // Customer
  custCard: { flexDirection: 'row', alignItems: 'center', gap: 12 },
