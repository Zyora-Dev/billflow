...
export default function CreditNoteListScreen({ navigation }: { navigation: any }) {
  ...
  const fetchData = useCallback(async () => {
    if (!orgId) return;
    try {
      const res = await api.get(`/api/credit-notes?org_id=${orgId}`);
      let data = Array.isArray(res.data) ? res.data : (res.data?.data || []);

      // Compute stats from full data
      const totalValue = data.reduce((s: number, cn: any) => s + Number(cn.total || 0), 0);
      const issuedCount = data.filter((cn: any) => cn.status === 'Issued').length;
      setStats({ count: data.length, totalValue, issuedCount });

      // Filter by status
      if (status !== 'All') {
        data = data.filter((cn: any) => cn.status === status);
      }
      // Filter by search
      if (searchDebounced) {
        const q = searchDebounced.toLowerCase();
        data = data.filter((cn: any) =>
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    const initial = (item.customer_name || 'CN').trim().charAt(0).toUpperCase();
    return (
      <TouchableOpacity
        style={styles.card}
        activeOpacity={0.85}
        ...
        <View style={styles.cardMid}>
          ...
          <View style={styles.cardBotRow}>
            <View style={styles.cardMetaInline}>
              <Text style={styles.cnNum}>{item.cn_number}</Text>
              <View style={styles.metaDot} />
              <Text style={styles.date}>{item.cn_date}</Text>
            </View>
    ...
    <View>
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.heroTopRow}>
          <View style={{ flex: 1 }}>
            <Text style={styles.heroEyebrow}>Credit Notes</Text>
      ...
      <Text style={styles.sectionLabel}>All Credit Notes</Text>
    </View>
  );
      ...
      <FlatList
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <EmptyState icon="document-text-outline" title="No credit notes found" subtitle="Tap + to create one" />
          )
        }
        contentContainerStyle={{ paddingBottom: 100 }}
      />
      <TouchableOpacity style={styles.fab} activeOpacity={0.85} onPress={() => navigation.navigate('CreditNoteForm', {})}>
...
const styles = StyleSheet.create({
  ...
  custName: { fontSize: 14, fontWeight: '700', color: colors.text, flex: 1, marginRight: 8 },
  ...
  cnNum: { fontSize: 11, fontWeight: '600', color: colors.gray500 },
