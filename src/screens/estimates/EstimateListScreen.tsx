...
export default function EstimateListScreen({ navigation }: { navigation: any }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (oid) {
        const res = await api.get(`/api/estimates?org_id=${oid}`);
  ...
  const filtered = useMemo(() => {
    let data = estimates.filter(e => isWithin(e.estimate_date, period));
    if (status !== 'All') data = data.filter((e: any) => e.status === status);
    if (search.trim()) {
    ...
    return data;
  }, [estimates, status, search, period]);

  const stats = useMemo(() => {
    const scope = estimates.filter(e => isWithin(e.estimate_date, period));
    return {
      total: scope.length,
      value: scope.reduce((s: number, e: any) => s + (Number(e.total) || 0), 0),
      accepted: scope.filter((e: any) => e.status === 'Accepted').length,
      sent: scope.filter((e: any) => e.status === 'Sent').length,
      draft: scope.filter((e: any) => e.status === 'Draft').length,
    };
  }, [estimates, period]);
    ...
    try {
      const rows = filtered.map((e, i) => `
        <tr>
          ...
          <td style="text-align:right">₹${(Number(e.total) || 0).toLocaleString('en-IN')}</td>
        </tr>
      `).join('');
      const html = `
        ...
        <style>
          ...
          <h1>Estimates Report</h1>
          <div class="sub">Period: ${PERIODS.find(p => p.value === period)?.label} • Generated ${new Date().toLocaleString()}</div>
          <table>
            <thead><tr><th>#</th><th>Estimate #</th><th>Customer</th><th>Service</th><th>Date</th><th>Status</th><th style="text-align:right">Amount</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </body></html>
      `;
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    const svcColor = SERVICE_COLOR[svc] || '#6b7280';
    return (
      <TouchableOpacity
        style={styles.card}
        activeOpacity={0.85}
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.cardMetaRow}>
            <Text style={styles.estNum}>{item.estimate_number}</Text>
            <View style={styles.metaDot} />
            <Text style={styles.date}>{fmtDateShort(item.estimate_date)}</Text>
            <View style={{ flex: 1 }} />
            <StatusBadge status={item.status} />
          </View>
          <View style={styles.cardBottomRow}>
            {svc ? (
              <View style={[styles.svcBadge, { backgroundColor: svcColor + '15', borderColor: svcColor + '30' }]}>
                <Text style={[styles.svcText, { color: svcColor }]}>{svc}</Text>
              </View>
            ) : null}
            {item.gst_enabled === false ? (
              <View style={[styles.svcBadge, { backgroundColor: '#f59e0b15', borderColor: '#f59e0b30' }]}>
                <Text style={[styles.svcText, { color: '#f59e0b' }]}>No GST</Text>
              </View>
            ) : null}
            {item.valid_until ? (
              <Text style={styles.validUntil}>Valid: {fmtDateShort(item.valid_until)}</Text>
            ) : null}
          </View>
        </View>
      </TouchableOpacity>
      ...
      <FlatList
          ...
          <View>
            ...
            <View style={styles.hero}>
              ...
              <View style={styles.heroTopRow}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.heroEyebrow}>Total Estimates</Text>
        ...
        ListEmptyComponent={loading ? <SkeletonList count={6} /> : (
          <View style={{ paddingTop: 40 }}>
            <EmptyState icon="document-outline" title="No estimates" subtitle="Tap + to create one" />
          </View>
        )}
      />

      <TouchableOpacity style={styles.fab} activeOpacity={0.85} onPress={() => navigation.navigate('EstimateForm', {})}>
...
const styles = StyleSheet.create({
  ...
  hero: {
    ...
    borderRadius: 20, paddingHorizontal: spacing.md, paddingVertical: spacing.md, overflow: 'hidden',
  ...
  kpiRow: { flexDirection: 'row', marginTop: spacing.sm + 2, paddingTop: spacing.sm, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.18)' },
  ...
  chip: {
    ...
    backgroundColor: '#fff', borderWidth: 1, borderColor: '#eef0f5',
  },
  chipDot: { width: 6, height: 6, borderRadius: 3 },
  chipText: { fontSize: 12, fontWeight: '700', color: colors.gray600 },
  card: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff',
    marginHorizontal: spacing.md, marginBottom: 10, borderRadius: 16,
    paddingVertical: 12, paddingHorizontal: 12, borderWidth: 1, borderColor: '#eef0f5',
  },
  avatar: {
    width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center',
    borderWidth: 1.5, marginRight: 11,
  },
  ...
  estNum: { fontSize: 11.5, color: colors.primary, fontWeight: '800', letterSpacing: 0.2 },
  ...
  cardBottomRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 4 },
  svcBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 999, borderWidth: 1 },
  svcText: { fontSize: 10, fontWeight: '700' },
  validUntil: { fontSize: 10.5, color: colors.gray500, fontWeight: '500', fontStyle: 'italic' },
  fab: {
