...
const STATUSES = ['All', 'Draft', 'Received', 'Paid', 'Partially Paid', 'Overdue', 'Cancelled'];
const PAGE_SIZE = 20;
...
export default function PBListScreen({ navigation }: { navigation: any }) {
  ...
  const [stats, setStats] = useState({ billed: 0, paid: 0, payable: 0 });
  const [overallPayable, setOverallPayable] = useState(0);
  ...
  const fetchOverallPayable = useCallback(async () => {
    if (!orgId) return;
    try {
      const res = await api.get(`/api/reports/purchase-summary?org_id=${orgId}`);
      setOverallPayable(Number(res.data?.total_payable || 0));
    } catch {}
  }, [orgId]);

  useEffect(() => { fetchOverallPayable(); }, [fetchOverallPayable]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/purchase-bills?${parts.join('&')}`);
      const d = res.data || {};
      setBills(Array.isArray(d.data) ? d.data : []);
      setTotal(d.total || 0);
      setPages(d.pages || 1);
      setStats(d.stats || { billed: 0, paid: 0, payable: 0 });
    } catch {} finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const onRefresh = () => { setRefreshing(true); fetchData(); fetchOverallPayable(); };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const [billRes, bizRes] = await Promise.all([
        api.get(`/api/purchase-bills?${parts.join('&')}`),
        api.get('/api/business'),
      ]);
      const allBills = Array.isArray(billRes.data?.data) ? billRes.data.data : [];
      const biz = bizRes.data?.[0] || null;
      const html = buildPBListHTML({
        business: biz, bills: allBills,
        stats: billRes.data?.stats || stats,
        period_label: periodLabel(period),
        date_from: range.from, date_to: range.to,
        status, search: searchDebounced,
      });
      const { uri } = await Print.printToFileAsync({ html });
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, {
          mimeType: 'application/pdf',
          dialogTitle: `Purchase Bills ${periodLabel(period)}`,
          UTI: 'com.adobe.pdf',
        });
      }
    } catch (e: any) {
  ...
  const Header = (
    <View>
      <View style={styles.hero}>
        ...
        <View style={styles.heroStatsRow}>
          <View style={styles.heroStatItem}>
            <Text style={styles.heroStatLabel}>Billed</Text>
            <CurrencyText amount={stats.billed} style={styles.heroStatVal} />
          </View>
          <View style={styles.heroDivider} />
          <View style={styles.heroStatItem}>
            <Text style={styles.heroStatLabel}>Paid</Text>
            <CurrencyText amount={stats.paid} style={[styles.heroStatVal, { color: '#86efac' }]} />
          </View>
          <View style={styles.heroDivider} />
          <View style={styles.heroStatItem}>
      ...
      {/* Overall payables strip — independent of date filter */}
      <View style={styles.stripRow}>
        <TouchableOpacity style={[styles.overallStrip, { flex: 1 }]} activeOpacity={0.85} onPress={() => navigation.navigate('Payables')}>
          ...
          <CurrencyText amount={overallPayable} style={styles.overallStripValue} />
      ...
      <View style={styles.pillsRow}>
        <TouchableOpacity
          ...
          activeOpacity={0.85}
          onPress={() => navigation.getParent()?.navigate('PurchasePayments')}
        >
        ...
        >
          ...
          <Text style={styles.actionPillText}>Vendors</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.actionPill}
          activeOpacity={0.85}
          onPress={() => navigation.getParent()?.navigate('PurchaseOrders')}
        >
      ...
      <FlatList
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <EmptyState icon="newspaper-outline" title="No purchase bills" subtitle="Try a different filter or tap + to create one" />
          )
        }
        contentContainerStyle={{ paddingBottom: 100 }}
      />
      <TouchableOpacity style={styles.fab} activeOpacity={0.85} onPress={() => navigation.navigate('PBForm', {})}>
          ...
          <TouchableOpacity activeOpacity={1} style={styles.modalSheet}>
            ...
            {draft.type === 'custom' && (
              <View style={{ gap: spacing.sm, marginTop: spacing.sm }}>
                <DateInput label="From" value={draft.type === 'custom' ? draft.from : ''} onChange={(v) => setDraft(d => d.type === 'custom' ? { ...d, from: v } : d)} />
                <DateInput label="To" value={draft.type === 'custom' ? draft.to : ''} onChange={(v) => setDraft(d => d.type === 'custom' ? { ...d, to: v } : d)} />
              </View>
            )}

            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.btnGhost} onPress={resetDraft}><Text style={styles.btnGhostText}>Reset</Text></TouchableOpacity>
              <TouchableOpacity style={styles.btnPrimary} onPress={applyDraft}><Text style={styles.btnPrimaryText}>Apply</Text></TouchableOpacity>
            </View>
          </TouchableOpacity>
        </TouchableOpacity>
      </Modal>
...
function buildPBListHTML(opts: {
  business: any; bills: any[]; stats: { billed: number; paid: number; payable: number };
  period_label: string; date_from: string; date_to: string; status: string; search: string;
}): string {
  const { business, bills, stats, period_label, date_from, date_to, status, search } = opts;
  ...
  const rows = bills.map((b, idx) => {
    ...
    const balColor = b.balance_due > 0 ? '#dc2626' : '#059669';
    return `<tr style="background:${bg}">
      <td>${escHtml(b.bill_number || '')}</td>
      <td>${fmtDateShort(b.bill_date)}</td>
      <td>${escHtml(b.vendor_name || '—')}</td>
      <td style="text-align:center"><span class="badge badge-${escHtml((b.status || '').toLowerCase().replace(/\s/g, '-'))}">${escHtml(b.status || '')}</span></td>
      <td class="num">${fmtAmt(b.total)}</td>
      <td class="num">${fmtAmt(b.amount_paid)}</td>
      <td class="num bold" style="color:${balColor}">${fmtAmt(b.balance_due)}</td>
    </tr>`;
  }).join('');
...
<style>
  *{box-sizing:border-box}
  ...
  .sum-card.billed{background:#fef3c7}
  .sum-card.paid{background:#dcfce7}
  .sum-card.payable{background:#fee2e2}
  ...
  .sum-val.billed{color:#b45309}
  .sum-val.paid{color:#15803d}
  .sum-val.payable{color:#dc2626}
  ...
  <div class="header">
    ...
    <div style="flex:1">
      <div class="biz-name">${escHtml(business?.business_name || 'Purchase Report')}</div>
      <div class="biz-meta">${meta.join('<br/>')}</div>
    </div>
  </div>
  <div class="title">PURCHASE BILLS REPORT</div>
  <div class="subtitle">${escHtml(period_label)} &nbsp;•&nbsp; ${fmtDateShort(date_from)} to ${fmtDateShort(date_to)}</div>
  <div class="summary">
    <div class="sum-card billed"><div class="sum-label">Billed</div><div class="sum-val billed">₹${fmtAmt(stats.billed)}</div></div>
    <div class="sum-card paid"><div class="sum-label">Paid</div><div class="sum-val paid">₹${fmtAmt(stats.paid)}</div></div>
    <div class="sum-card payable"><div class="sum-label">Payable</div><div class="sum-val payable">₹${fmtAmt(stats.payable)}</div></div>
  </div>
  ${filters.length ? `<div class="filters"><strong>Filters:</strong> ${filters.join(' &nbsp;•&nbsp; ')}</div>` : ''}
  <table>
    <thead><tr>
      <th>Bill #</th><th>Date</th><th>Vendor</th>
      <th class="center">Status</th>
      <th class="num">Total</th><th class="num">Paid</th><th class="num">Balance</th>
    </tr></thead>
    <tbody>${rows || `<tr><td colspan="7" style="text-align:center;padding:30px;color:#9ca0ad">No bills in this period</td></tr>`}</tbody>
    <tfoot><tr>
      <td colspan="4">TOTAL (${bills.length} bills)</td>
      <td class="num">₹${fmtAmt(totalBilled)}</td>
...
const styles = StyleSheet.create({
  ...
  overallStripValue: { flex: 1, fontSize: 14, fontWeight: '800', color: '#dc2626', letterSpacing: -0.3 },

  pillsRow: { flexDirection: 'row', gap: 6, marginHorizontal: spacing.md, marginTop: spacing.sm + 2 },
  actionPill: {
  ...
  actionPillText: { fontSize: 12, fontWeight: '800', color: colors.primary, letterSpacing: 0.2 },
  ...
  monthChip: { width: '23%', paddingVertical: 10, alignItems: 'center', backgroundColor: colors.gray100, borderRadius: 10 },
