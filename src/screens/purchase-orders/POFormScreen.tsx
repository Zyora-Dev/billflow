...
export default function POFormScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (editId) {
          const po = (await api.get(`/api/purchase-orders/${editId}`)).data;
          setVendorId(po.vendor_id);
          setVendorName(po.vendor_name || '');
          setPoDate(po.po_date || '');
          setValidUntil(po.valid_until || '');
          setNotes(po.notes || '');
          setDiscountType(po.discount_type || 'flat');
          setDiscountValue(String(po.discount_value || 0));
  ...
  const handleSave = async () => {
    ...
    try {
      const body = {
        org_id: orgId, vendor_id: vendorId, po_date: poDate, valid_until: validUntil || null,
      ...
      if (editId) await api.put(`/api/purchase-orders/${editId}`, body);
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <DateInput label="PO Date" value={poDate} onChange={setPoDate} placeholder="Select date" />
            </View>
        ...
        <TouchableOpacity style={[styles.saveBtn, loading && { opacity: 0.6 }]} onPress={handleSave} disabled={loading}>
          <Text style={styles.saveBtnText}>{loading ? 'Saving...' : editId ? 'Update PO' : 'Create Purchase Order'}</Text>
        </TouchableOpacity>
