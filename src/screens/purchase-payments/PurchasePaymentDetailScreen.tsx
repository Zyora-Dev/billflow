...
export default function PurchasePaymentDetailScreen({ route, navigation }: { route: any; navigation: any }) {
...
<style>
  *{box-sizing:border-box}
  ...
  .amount-card{background:#fee2e2;border:2px solid #dc2626;border-radius:10px;padding:20px;text-align:center;margin-bottom:20px}
  ...
  <div class="header">
    ...
    <div style="flex:1">
      <div class="biz-name">${escHtml(business?.business_name || 'Voucher')}</div>
      <div class="biz-meta">${meta.join('<br/>')}</div>
    </div>
    <div class="receipt-no">
      Voucher #<br/>
      <span class="num">VCH-${String(payment.id).padStart(5, '0')}</span><br/>
  ...
  <div class="signature">
    <div class="sig-block"><div class="sig-line">Vendor's Signature</div></div>
    <div class="sig-block"><div class="sig-line">Authorised Signatory</div></div>
  </div>

  <div class="footer">This is a computer-generated voucher. Generated on ${fmtDate(new Date().toISOString())}.</div>
</body></html>`;
  };
    ...
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, {
          ...
          dialogTitle: `Voucher VCH-${payment?.id}`,
          UTI: 'com.adobe.pdf',
        });
      }
    } catch (e: any) {
    ...
    <ScrollView style={s.container} contentContainerStyle={{ padding: spacing.md, paddingBottom: 40 }}>
      ...
      <View style={s.section}>
        ...
        <View style={s.partyRow}>
          <View style={s.avatar}>
            <Text style={s.avatarText}>{(payment.vendor_name || '?').charAt(0).toUpperCase()}</Text>
          </View>
          <View style={{ flex: 1 }}>
      ...
      <View style={s.section}>
        ...
        <View style={[s.row, { borderBottomWidth: 0 }]}>
          ...
          <Text style={s.rowVal}>VCH-{String(payment.id).padStart(5, '0')}</Text>
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  heroCard: {
    ...
    shadowColor: colors.danger, shadowOpacity: 0.3, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 5,
  },
  heroIconWrap: {
