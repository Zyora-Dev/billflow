...
export default function PurchasePaymentListScreen({ navigation }: { navigation: any }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (vendorId) parts.push(`vendor_id=${vendorId}`);
      const [payRes, venRes] = await Promise.all([
        api.get(`/api/purchase-payments?${parts.join('&')}`),
  ...
  const filtered = useMemo(() => {
    ...
    return payments.filter((p: any) => {
      ...
      if (q) {
        const hay = `${p.reference_number || ''} ${p.payment_method || ''} ${p.notes || ''} ${p.vendor_name || ''} ${p.bill_number || ''}`.toLowerCase();
  ...
  const vendorName = vendorId
    ? vendors.find(v => v.id === vendorId)?.business_name
  ...
  const buildHTML = () => {
    ...
    if (vendorId) filters.push(`Vendor: ${escHtml(vendorName)}`);
...
<style>
  *{box-sizing:border-box}
  ...
  <div class="summary">
    <div class="sum-card"><div class="sum-label">Total Paid Out</div><div class="sum-val">₹${fmtAmt(stats.total)}</div></div>
  ...
  <table>
    <thead><tr>
      <th>Date</th><th>Vendor</th><th>Bill #</th><th>Method</th><th>Reference</th>
          ...
          <>
            <View style={s.hero}>
              ...
              <View style={s.heroTopRow}>
                <View style={{ flex: 1 }}>
                  <Text style={s.heroEyebrow}>Total Paid Out</Text>
              ...
              <View style={s.heroStatsRow}>
                ...
                <View style={s.heroStatItem}>
                  ...
                  <CurrencyText amount={stats.bankAmt} style={[s.heroStatVal, { color: '#fca5a5' }]} />
                </View>
                <View style={s.heroDivider} />
                <View style={s.heroStatItem}>
      ...
      <Modal visible={showVendorPicker} animationType="slide" onRequestClose={() => setShowVendorPicker(false)}>
        <View style={s.fullModal}>
          <View style={s.fullModalHeader}>
            <Text style={s.modalTitle}>Filter by Vendor</Text>
          ...
          <FlatList
            data={[{ id: null, label: 'All vendors' }, ...filteredVendors] as any[]}
            keyExtractor={(it: any) => String(it.id ?? 'all')}
            keyboardShouldPersistTaps="handled"
            renderItem={({ item }: any) => {
              const isSelected = vendorId === item.id;
              const label = item.label || item.business_name || item.contact_person || '—';
              return (
                <TouchableOpacity
                  style={[s.vendorItem, isSelected && { backgroundColor: colors.primary + '10' }]}
                  onPress={() => { setVendorId(item.id); setShowVendorPicker(false); setVendorSearch(''); }}
                >
                  <Text style={s.vendorItemText}>{label}</Text>
...
const s = StyleSheet.create({
  ...
  vendorItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.gray100 },
  vendorItemText: { fontSize: fontSize.md, color: colors.text, fontWeight: '500' },
});
