...
export default function QuotationFormScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <Modal visible={showItemPicker} animationType="slide" onRequestClose={() => setShowItemPicker(false)}><View style={styles.modalC}><View style={styles.modalH}><Text style={styles.modalT}>Add Item</Text><TouchableOpacity hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} style={{ padding: 8 }} onPress={() => setShowItemPicker(false)}><Ionicons name="close" size={24} color={colors.text} /></TouchableOpacity></View><TouchableOpacity style={styles.customItemBtn} onPress={addCustomItem}><Ionicons name="create-outline" size={20} color={colors.primary} /><Text style={styles.customItemText}>Add Custom Item (not in catalog)</Text></TouchableOpacity><View style={{ flexDirection: 'row', gap: 8, paddingHorizontal: 16, marginBottom: 8 }}><TextInput style={[styles.modalS, { flex: 1, marginHorizontal: 0, marginBottom: 0, color: colors.text }]} value={search} onChangeText={setSearch} placeholder="Search..." placeholderTextColor={colors.placeholder} /><TextInput style={[styles.modalS, { width: 130, marginHorizontal: 0, marginBottom: 0, color: colors.text }]} placeholder="Scan barcode..." placeholderTextColor={colors.placeholder} returnKeyType="done" onSubmitEditing={async (e: any) => { const barcode = e.nativeEvent.text.trim(); if (!barcode || !orgId) return; try { const res = await api.get(`/api/items/lookup/barcode?barcode=${encodeURIComponent(barcode)}&org_id=${orgId}`); if (res.data) { addItem(res.data); } else { Alert.alert('Not Found', 'No item for this barcode'); } } catch { Alert.alert('Not Found', 'No item for this barcode'); } }} /></View><FlatList data={items.filter(i => !search || i.item_name?.toLowerCase().includes(search.toLowerCase()))} keyExtractor={i => String(i.id)} keyboardShouldPersistTaps="handled" renderItem={({ item }) => (<TouchableOpacity style={styles.modalI} onPress={() => addItem(item)}><Text style={styles.modalIT}>{item.item_name}</Text><Text style={styles.modalIS}>₹{item.sale_price} • {item.tax_rate}%</Text></TouchableOpacity>)} ListEmptyComponent={<View style={{ padding: 16, alignItems: 'center' }}><Ionicons name="cube-outline" size={32} color={colors.gray400} /><Text style={{ color: colors.gray500, marginTop: 8 }}>No items found.</Text></View>} /></View></Modal>
