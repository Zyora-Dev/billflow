...
export default function QuotationListScreen({ navigation }: { navigation: any }) {
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      data = data.filter((i: any) => i.quotation_number?.toLowerCase().includes(q) || i.customer_name?.toLowerCase().includes(q));
    }
    setFiltered(data);
  }, [status, quotes, search]);
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('QuotationDetail', { id: item.id })} onLongPress={() => preview.show({ type: 'quotation', id: item.id })} delayLongPress={350}>
      <View style={{ flex: 1 }}>
        <Text style={styles.num}>{item.quotation_number}</Text>
        <Text style={styles.cust}>{item.customer_name || 'N/A'}</Text>
        <Text style={styles.date}>{item.quotation_date}</Text>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statsRow}>
        <View style={styles.stat}><Text style={styles.statLabel}>Total</Text><Text style={styles.statVal}>{statsData.count}</Text></View>
      ...
      <View style={styles.searchRow}>
        ...
        <TextInput style={styles.searchInput} value={search} onChangeText={setSearch} placeholder="Search quotations..." placeholderTextColor={colors.placeholder} />
      ...
      <FlatList
        ...
        ListEmptyComponent={loading ? <SkeletonList count={6} /> : <EmptyState icon="document-outline" title="No quotations" subtitle="Tap + to create one" />}
        contentContainerStyle={!filtered.length ? { flex: 1 } : { paddingBottom: 80 }}
      />
      <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('QuotationForm', {})}>
