...
export default function ReceiptDetailScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  const receiptNo = `RCT-${String(payment?.id || 0).padStart(5, '0')}`;

  const handleDelete = () => {
    Alert.alert('Delete Receipt?', 'This will reverse the invoice allocation. Cannot be undone.', [
...
<style>
  *{box-sizing:border-box}
  ...
  .header{display:flex;align-items:flex-start;gap:14px;padding-bottom:14px;border-bottom:2px solid #059669}
  .logo{width:64px;height:64px;object-fit:contain}
  .biz-name{font-size:22px;font-weight:800;color:#059669;margin:0;letter-spacing:-0.3px}
  ...
  .receipt-no .num{font-size:14px;font-weight:800;color:#059669;letter-spacing:0.5px}
  .title{text-align:center;font-size:22px;font-weight:800;color:#059669;letter-spacing:3px;margin:28px 0 8px}
  .title-rule{height:2px;background:#059669;width:80px;margin:0 auto 28px}
  .info-grid{display:flex;gap:16px;margin-bottom:24px}
  .info-card{flex:1;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:14px 16px}
  ...
  .badge{display:inline-block;padding:3px 10px;border-radius:999px;font-size:10px;font-weight:700;background:#dcfce7;color:#15803d}
  ...
  <div class="header">
    ...
    <div class="receipt-no">
      Receipt #<br/>
      <span class="num">${receiptNo}</span><br/>
  ...
  const handlePDF = async () => {
    ...
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: `Receipt ${receiptNo}`, UTI: 'com.adobe.pdf' });
      }
    } catch (e: any) {
  ...
  if (!payment) {
    return (
      <View style={[s.container, { justifyContent: 'center', alignItems: 'center', padding: spacing.lg }]}>
        ...
        <Text style={{ marginTop: spacing.md, color: colors.gray600 }}>Receipt not found</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={{ padding: spacing.md, paddingBottom: 40 }}>
      ...
      <View style={s.receiptStrip}>
        ...
        <Text style={s.receiptStripText}>{receiptNo}</Text>
      </View>
      ...
      <View style={s.section}>
        ...
        <View style={[s.row, { borderBottomWidth: 0 }]}>
          ...
          <Text style={s.rowVal}>{receiptNo}</Text>
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  receiptStripText: { fontSize: fontSize.sm, fontWeight: '800', color: colors.primary, letterSpacing: 0.5 },
