...
export default function ReceiptListScreen({ navigation }: { navigation: any }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setOrgId(oid);
      let url = `/api/payments?org_id=${encodeURIComponent(oid)}`;
  ...
  const filtered = useMemo(() => {
    ...
    return payments.filter((p: any) => {
      const hay = `${p.customer_name || ''} ${p.reference_number || ''} ${p.invoice_number || ''} ${p.notes || ''}`.toLowerCase();
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.hero}>
        ...
        <Text style={styles.heroCount}>{filtered.length} receipt{filtered.length !== 1 ? 's' : ''}</Text>
      </View>
