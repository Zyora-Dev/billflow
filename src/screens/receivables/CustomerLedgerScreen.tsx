...
export default function CustomerLedgerScreen({ route, navigation }: { route: any; navigation: any }) {
  const { customer_id, customer_name, receivables_balance } = route.params || {};
  ...
  useEffect(() => {
    navigation.setOptions({ title: customer_name || 'Ledger' });
  }, [navigation, customer_name]);

  const fetchAll = useCallback(async (oid?: string | null) => {
    const id = oid || orgId;
    if (!id || !customer_id) return;
    try {
      const [stmt, bal, cust] = await Promise.all([
        api.get(`/api/ledger/customer-statement/${customer_id}?org_id=${id}`),
        api.get(`/api/balance/customer/${customer_id}?org_id=${id}`),
        api.get(`/api/customers/${customer_id}`),
      ]);
      setEntries(Array.isArray(stmt.data?.entries) ? stmt.data.entries : []);
      setSummary(bal.data || null);
      setParty(cust.data || null);
    } catch {} finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const partyInfo = useMemo(() => ({
    name: party?.business_name || party?.contact_person || customer_name || 'Customer',
  ...
  const handleCall = () => {
    if (!partyInfo.mobile) return Alert.alert('No mobile', 'Customer has no mobile number on file.');
    Linking.openURL(`tel:${partyInfo.mobile}`).catch(() => {});
  };

  // Ledger's closing running balance is the authoritative outstanding.
  // It anchors to Tally closing (when synced) and includes prior-FY opening B/F.
  ...
  const opening = openingEntry ? Number(openingEntry.debit || 0) - Number(openingEntry.credit || 0) : 0;

  const tagFor = (type: string) => {
    if (type === 'Opening Balance') return { bg: '#ede9fe', fg: '#7c3aed', icon: 'time' as const };
    if (type === 'Invoice') return { bg: '#dbeafe', fg: '#1d4ed8', icon: 'document-text' as const };
    if (type === 'Receipt') return { bg: '#dcfce7', fg: '#15803d', icon: 'cash' as const };
    if (type === 'Credit Note') return { bg: '#fee2e2', fg: '#dc2626', icon: 'arrow-undo' as const };
    ...
    return (
      <View style={[styles.entry, isOpening && styles.entryOpening]}>
        ...
        <View style={{ alignItems: 'flex-end' }}>
          {item.debit > 0 && (
            <Text style={[styles.entryAmt, { color: '#dc2626' }]}>+ ₹{Number(item.debit).toLocaleString('en-IN')}</Text>
          )}
          {item.credit > 0 && (
            <Text style={[styles.entryAmt, { color: '#15803d' }]}>− ₹{Number(item.credit).toLocaleString('en-IN')}</Text>
          )}
  ...
  const Header = (
    <View>
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.heroTopRow}>
          <View style={styles.avatar}>
            <Text style={styles.avatarText}>
              {String(customer_name || 'U').trim().charAt(0).toUpperCase()}
            </Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.heroLabel}>Closing Balance</Text>
            ...
            {openingEntry ? (
              ...
              <View style={styles.sourcePill}>
                <Ionicons name="document-text" size={10} color="rgba(255,255,255,0.95)" />
        ...
        {/* Opening B/F + Movement breakdown */}
      ...
      {/* Reconciliation note if Receivables list value differs from ledger closing */}
      {typeof receivables_balance === 'number' && Math.abs(Number(receivables_balance) - closingBalance) > 1 && (
        <View style={styles.reconcileCard}>
          <Ionicons name="information-circle" size={16} color="#b45309" />
          <View style={{ flex: 1 }}>
            <Text style={styles.reconcileTitle}>Receivables shows ₹{Number(receivables_balance).toLocaleString('en-IN')}</Text>
            <Text style={styles.reconcileSub}>
              Ledger closing differs because it includes
              {openingEntry ? ' opening B/F from previous FY' : ' unallocated receipts/advances'}.
...
const styles = StyleSheet.create({
  ...
  heroLabel: { color: 'rgba(255,255,255,0.7)', fontSize: 11, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.4 },
  ...
  heroStatItem: { flex: 1 },
  heroStatLabel: { color: 'rgba(255,255,255,0.65)', fontSize: 10, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.3 },
  heroStatVal: { color: '#ffffff', fontSize: 14, fontWeight: '800', marginTop: 2 },
  heroStatSub: { color: 'rgba(255,255,255,0.5)', fontSize: 9, marginTop: 1, fontWeight: '600' },
  heroDivider: { width: 1, height: 36, backgroundColor: 'rgba(255,255,255,0.15)', marginHorizontal: spacing.sm },
  ...
  breakdownLabel: { color: 'rgba(255,255,255,0.6)', fontSize: 9, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.3 },
  ...
  entryCountChip: { marginLeft: 'auto', paddingHorizontal: 8, paddingVertical: 2, backgroundColor: colors.primary + '15', borderRadius: 999 },
  entryCountText: { fontSize: 10, fontWeight: '800', color: colors.primary },
