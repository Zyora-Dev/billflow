...
export default function PurchaseBillSettingsScreen({ navigation }: any) {
  ...
  const [billTitle, setBillTitle] = useState('Purchase Bill');
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setOrgId(oid);
        const r = await api.get(`/api/purchase-bill-settings?org_id=${oid}`);
        const s = r.data;
        setBillPrefix(s.bill_prefix || 'BILL-');
        setNextNumber(String(s.next_number || 1));
        setBillTitle(s.bill_title || 'Purchase Bill');
        ...
        if (s.header_logo) setLogoUri(`${BASE}/assets/purchase/${s.header_logo}`);
        if (s.signature_image) setSignatureUri(`${BASE}/assets/purchase/${s.signature_image}`);
        if (s.qr_code_image) setQrUri(`${BASE}/assets/purchase/${s.qr_code_image}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fd.append('bill_prefix', billPrefix);
      fd.append('next_number', nextNumber);
      fd.append('bill_title', billTitle);
      ...
      await api.put('/api/purchase-bill-settings', fd, {
  ...
  return (
    <ScrollView style={st.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Choose a style for your purchase bills</Text>
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Logo and title shown on purchase bills</Text>
        ...
        <Text style={[st.label, { marginTop: 16 }]}>Bill Title</Text>
        <TextInput style={st.input} value={billTitle} onChangeText={setBillTitle} placeholder="Purchase Bill" placeholderTextColor={colors.gray400} />
      </View>

      {/* Numbering */}
      <View style={st.card}>
        <Text style={st.cardTitle}>Numbering</Text>
        <Text style={st.cardSub}>Auto-generated bill number format</Text>
        <View style={{ flexDirection: 'row', gap: 10 }}>
          <View style={{ flex: 1 }}>
            <Text style={st.label}>Prefix</Text>
            <TextInput style={st.input} value={billPrefix} onChangeText={setBillPrefix} placeholder="BILL-" placeholderTextColor={colors.gray400} />
          </View>
          <View style={{ flex: 1 }}>
        ...
        <View style={st.previewBox}>
          ...
          <Text style={st.previewText}>Preview: {billPrefix}{String(nextNumber).padStart(5, '0')}</Text>
        </View>
      </View>
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Shown on purchase bill for payment reference</Text>
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Payment QR shown on purchase bills</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          {qrUri ? (
            <View style={st.imgWrap}>
