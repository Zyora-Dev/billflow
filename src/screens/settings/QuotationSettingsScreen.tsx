...
export default function QuotationSettingsScreen({ navigation }: any) {
  const toast = useToast();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [orgId, setOrgId] = useState('');

  const [quotePrefix, setQuotePrefix] = useState('QTN-');
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setOrgId(oid);
        const r = await api.get(`/api/quotation-settings?org_id=${oid}`);
        const s = r.data;
        setQuotePrefix(s.quote_prefix || 'QTN-');
        setNextNumber(String(s.next_number || 1));
        setQuoteTitle(s.quote_title || 'Quotation');
        ...
        if (s.header_logo) setLogoUri(`${BASE}/assets/quotation/${s.header_logo}`);
        if (s.signature_image) setSignatureUri(`${BASE}/assets/quotation/${s.signature_image}`);
        if (s.qr_code_image) setQrUri(`${BASE}/assets/quotation/${s.qr_code_image}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      fd.append('quote_prefix', quotePrefix);
      fd.append('next_number', nextNumber);
      fd.append('quote_title', quoteTitle);
      ...
      await api.put('/api/quotation-settings', fd, {
  ...
  return (
    <ScrollView style={st.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Choose a style for your quotations</Text>
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Logo and title shown on quotations</Text>
        ...
        <Text style={[st.label, { marginTop: 16 }]}>Quotation Title</Text>
        <TextInput style={st.input} value={quoteTitle} onChangeText={setQuoteTitle} placeholder="Quotation" placeholderTextColor={colors.gray400} />
      </View>

      {/* Numbering */}
      <View style={st.card}>
        <Text style={st.cardTitle}>Numbering</Text>
        <Text style={st.cardSub}>Auto-generated quotation number format</Text>
        <View style={{ flexDirection: 'row', gap: 10 }}>
          <View style={{ flex: 1 }}>
            <Text style={st.label}>Prefix</Text>
            <TextInput style={st.input} value={quotePrefix} onChangeText={setQuotePrefix} placeholder="QTN-" placeholderTextColor={colors.gray400} />
          </View>
          <View style={{ flex: 1 }}>
        ...
        <View style={st.previewBox}>
          ...
          <Text style={st.previewText}>Preview: {quotePrefix}{String(nextNumber).padStart(5, '0')}</Text>
        </View>
      </View>
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Shown on quotation for payment reference</Text>
      ...
      <View style={st.card}>
        ...
        <Text style={st.cardSub}>Payment QR shown on quotations</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          {qrUri ? (
            <View style={st.imgWrap}>
